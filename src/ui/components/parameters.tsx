...
export const Parameters = observer(() => {
    ...
    return (
        <>
            ...
            <RangeRow label={`Mutation base organs`} min={0} max={100} step={1} onChange={(v) => parameters.setMutationBaseOrgansRate(v)} value={parameters.getMutationBaseOrgansRate()} />
            <RangeRow label={`Mutation limb organs`} min={0} max={100} step={1} onChange={(v) => parameters.setMutationLimbOrgansRate(v)} value={parameters.getMutationLimbOrgansRate()} />
        </>
    );
});